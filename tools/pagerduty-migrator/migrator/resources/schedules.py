...
class Layer:
    ...
    def to_oncall_shifts(
        ...
        if (frequency, restrictions_type) in (
            ...
            shifts, is_split = self._generate_shifts(unique=False)
    ...
    def _generate_shifts(
        ...
        Also returns a boolean indicating whether the shifts are split by restrictions.
        "Split by restrictions" means there's a shift not fully covered by any restriction due to an on-call handoff.
        """
        ...
        while current < end:
            ...
            if (shift_start == start or shift_end == end) and (
                shift_start > restriction_start
            ):
                is_split = True
            ...
            if not unique or (shift[0].time(), shift[1].time()) not in [
                (s[0].time(), s[1].time()) for s in shifts
            ]:
                shifts.append(shift)
