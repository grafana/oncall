...
def match_integration_type(integration: dict, vendors: list[dict]) -> None:
    if integration["vendor"] != None:
        ...
        integration["name"] = PAGERDUTY_TO_ONCALL_VENDOR_MAP.get(vendor_name)
...
def migrate_integration(integration: dict, escalation_policies: list[dict]) -> None:
    ...
    if "oncall_type" in integration.keys():
        oncall_type = integration["oncall_type"]
    else:
        oncall_type = "webhook"
