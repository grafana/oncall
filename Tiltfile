...
# load .env file, right now only needed to read ONCALL_IMAGE_TARGET
...
oncall_image_build_target = os.getenv("ONCALL_IMAGE_TARGET", "dev")
running_oncall_enterprise = oncall_image_build_target == "dev-enterprise"
...
engine_pod_mount_dir = "/etc/app"
local_engine_dir_name = "engine"
local_engine_dir_relative = "./{}/".format(local_engine_dir_name)

engine_live_update_steps = [sync(local_engine_dir_relative, engine_pod_mount_dir)]
if running_oncall_enterprise:
    local_engine_enterprise_dir = "extensions/engine_enterprise"
    absolute_enterprise_engine_dir = "{}/{}/{}/".format(cwd, local_engine_dir_name, local_engine_enterprise_dir)

    engine_live_update_steps += [
        sync(absolute_enterprise_engine_dir, "{}/{}".format(engine_pod_mount_dir, local_engine_enterprise_dir)),
        run(
            "cd {} && pip install -r requirements-enterprise-docker.txt".format(local_engine_dir_relative),
            trigger="{}requirements-enterprise-docker.txt".format(local_engine_dir_relative),
        ),
    ]

engine_live_update_steps += [
    run(
        "cd {} && pip install -r requirements.txt".format(local_engine_dir_relative),
        trigger="{}requirements.txt".format(local_engine_dir_relative),
    ),
]

print(engine_live_update_steps)
...
k8s_resource(
    ...
    port_forwards='6379:6379', # <host_port>:<container_port>
    labels=["OnCallDeps"],
)
k8s_resource(
    workload="mariadb",
