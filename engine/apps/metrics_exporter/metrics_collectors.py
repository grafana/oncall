...
from settings.base import (
    ALERT_GROUPS_AND_NOTIFICATIONS_METRIC_COLLECTOR_ENABLED,
    RESPONSE_TIME_METRIC_COLLECTOR_ENABLED,
)
...
class BaseApplicationMetricsCollector(ABC):
    def __init__(self):
    ...
    @abstractmethod
    def get_metrics_and_missing_orgs(self, org_ids: set) -> typing.Tuple[typing.List[Metric], set]:
        raise NotImplementedError

    def collect(self):
        org_ids = set(get_organization_ids())

        metrics, missing_org_ids = self.get_metrics_and_missing_orgs(org_ids)
...
class AlertGroupsAndNotificationsMetricsCollector(BaseApplicationMetricsCollector):
    """Collects ALERT_GROUPS_TOTAL and USER_WAS_NOTIFIED_OF_ALERT_GROUPS metrics"""
    ...
    def get_metrics_and_missing_orgs(self, org_ids: set) -> typing.Tuple[typing.List[Metric], set]:
        ...
        user_was_notified, missing_org_ids_2 = self._get_user_was_notified_of_alert_groups_metric(org_ids)
        ...
        return [alert_groups_total, user_was_notified], missing_org_ids
...
class AlertGroupResponseTimeMetricsCollector(BaseApplicationMetricsCollector):
    """Collects ALERT_GROUPS_RESPONSE_TIME metric"""

    def get_metrics_and_missing_orgs(self, org_ids: set) -> typing.Tuple[typing.List[Metric], set]:
        # alert groups response time metrics: histogram
        alert_groups_response_time_seconds, missing_org_ids = self._get_response_time_metric(org_ids)
            ...
            for _, integration_data in ag_response_time.items():
                ...
                for service_name, response_time in integration_data["services"].items():
                    if not response_time:
                        continue
                    buckets_values, sum_value = self.get_buckets_with_sum(response_time)
    ...
    def get_buckets_with_sum(self, values: typing.List[int]) -> typing.Tuple[typing.Dict[str, int], typing]:
...
if ALERT_GROUPS_AND_NOTIFICATIONS_METRIC_COLLECTOR_ENABLED:
    application_metrics_registry.register(AlertGroupsAndNotificationsMetricsCollector())
if RESPONSE_TIME_METRIC_COLLECTOR_ENABLED:
    application_metrics_registry.register(AlertGroupResponseTimeMetricsCollector())
