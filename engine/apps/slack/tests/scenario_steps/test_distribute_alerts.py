...
from apps.slack.scenarios.distribute_alerts import CreateAlertGroupStep
...
def test_skip_escalations_error(
    ...
    SlackCreateAlertGroupStep = ScenarioStep.get_step("distribute_alerts", "CreateAlertGroupStep")
    ...
    step = SlackCreateAlertGroupStep(slack_team_identity)
...
@patch.object(CreateAlertGroupStep, "_post_alert_group_to_slack")
@pytest.mark.django_db
def test_create_alert_group_step_no_channel_filter(
    ...
    step = CreateAlertGroupStep(slack_team_identity, organization)
    step.process_signal(alert)
