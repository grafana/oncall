...
class SocialAuthAuthCanceledExceptionMiddleware(SocialAuthExceptionMiddleware):
    def process_exception(self, request, exception):
        ...
        if isinstance(exception, exceptions.AuthCanceled):
            ...
            return redirect(organization.build_absolute_plugin_ui_url(redirect_to))
            ...
            return redirect(organization.build_absolute_plugin_ui_url(f"{redirect_to}&slack_error={SLACK_AUTH_FAILED}"))
            ...
            return redirect(
                organization.build_absolute_plugin_ui_url(f"{redirect_to}?tab=Slack&slack_error={SLACK_REGION_ERROR}")
            )
