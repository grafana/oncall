...
@pytest.mark.parametrize(
    "swap_end,expected",
    [
        (ROTATION_START + datetime.timedelta(hours=12), "00h00 - 12h00 (UTC) on Sat August 29, 2150"),
        (
            ROTATION_START + datetime.timedelta(hours=24 * 5),
            "00h00 - 12h00 (UTC) from Sat August 29, 2150 until Wed September 2, 2150",
        ),
    ],
)
def test_shifts_summary_simple_rotations(
    ...
    on_call_shift.add_rolling_users([[beneficiary], [benefactor]])

    ssr = make_shift_swap_request(schedule, beneficiary, swap_start=ROTATION_START, swap_end=swap_end)

    assert ssr.shifts_summary == expected


@pytest.mark.django_db
def test_shifts_summary_simple_rotations_swap_starts_halfway_into_first_shift(
    make_organization_and_user,
    make_user_for_organization,
    make_schedule,
    make_on_call_shift,
    ...
    swap_start = ROTATION_START + datetime.timedelta(hours=6)
    swap_end = ROTATION_START + datetime.timedelta(hours=24 * 5)
    ssr = make_shift_swap_request(schedule, beneficiary, swap_start=swap_start, swap_end=swap_end)

    assert ssr.shifts_summary == "00h00 - 12h00 (UTC) from Sat August 29, 2150 until Wed September 2, 2150"


@pytest.mark.django_db
def test_shifts_summary_simple_rotations_swap_ends_halfway_into_last_shift(
    make_organization_and_user,
    make_user_for_organization,
    make_schedule,
    make_on_call_shift,
    ...
    swap_end = ROTATION_START + datetime.timedelta(hours=(24 * 4) + 6)
    ssr = make_shift_swap_request(schedule, beneficiary, swap_start=ROTATION_START, swap_end=swap_end)
...
@pytest.mark.django_db
def test_shifts_summary_complex_rotation_layers(
    make_organization_and_user,
    make_user_for_organization,
    make_schedule,
    make_on_call_shift,
    ...
    # TODO: we should consider adding some test cases where there are really complex rotation layers, for example
    # a user being on-call > 1x/day.. I don't think the shifts_summary property will currently support this
    pass
