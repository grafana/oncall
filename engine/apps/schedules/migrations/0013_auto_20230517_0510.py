...
def drop_autogenerated_web_shift_name(apps, schema_editor):
    ...
    shifts_to_update = []
    for shift in CustomOnCallShift.objects.filter(source=SOURCE_WEB):
        shift.name = f'L{shift.priority_level}'
        shifts_to_update.append(shift)
    CustomOnCallShift.objects.bulk_update(shifts_to_update, ["name"], 5000)
