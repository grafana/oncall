...
class LabelsCRUDView(APIView):
    ...
    def get(self, request):  # todo
        ...
        if key_id:
            result = LabelsAPIClient().get_labels_keys()
        else:
            result = LabelsAPIClient().get_label_key_values(key_id)
    ...
    def post(self, request):  # todo
        ...
        LabelsAPIClient().create_label(label_data)
    ...
    def put(self, request):  # todo
        ...
        # todo: update key name, update value name, add new value
        # {
        #     "key_id"
        #     "key_name"
        #     "value_id"
        ...
        if label_action == "key_update":
            key_name = self.request.data.get("key_name")
            if key_name:
                result = LabelsAPIClient().update_label_key(key_id, {"key_name": key_name})
                return Response(result)
            raise BadRequest
        elif label_action == "value_update":
            # value_id = self.request.query_params.get("valueID")
            # value_name = self.request.data.get("value_name")
            pass
