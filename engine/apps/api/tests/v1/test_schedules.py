...
from apps.api.serializers.v1.user import ScheduleUserSerializer
...
def test_create_ical_schedule(schedule_internal_api_setup, make_user_auth_headers):
    ...
    url = reverse("api-internal:schedule-list")
    with patch(
        "apps.api.serializers.v1.schedule_ical.ScheduleICalSerializer.validate_ical_url_primary", return_value=ICAL_URL
    ):
        data = {
            "ical_url_primary": ICAL_URL,
            "ical_url_overrides": None,
...
def test_create_invalid_ical_schedule(schedule_internal_api_setup, make_user_auth_headers):
    ...
    with patch(
        "apps.api.serializers.v1.schedule_ical.ScheduleICalSerializer.validate_ical_url_primary",
...
def test_schedule_create_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.create",
...
def test_schedule_update_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.update",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_schedule_list_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.list",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_schedule_retrieve_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.retrieve",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_schedule_delete_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.destroy",
        return_value=Response(
...
def test_events_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.events",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_reload_ical_permissions(
    ...
    with patch(
        "apps.api.views.v1.schedule.ScheduleView.reload_ical",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
