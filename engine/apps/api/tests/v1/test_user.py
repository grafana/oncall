...
def test_user_update_self_permissions(
    ...
    with patch(
        "apps.api.views.v1.user.UserView.update",
...
def test_user_list_permissions(
    ...
    url = reverse("api-internal:user-list")
    with patch(
        "apps.api.views.v1.user.UserView.list",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_user_get_own_verification_code(
    ...
    with patch(
        "apps.api.views.v1.user.UserView.get_verification_code",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_user_verify_own_phone(
    ...
    with patch(
        "apps.api.views.v1.user.UserView.verify_number",
        return_value=Response(
            status=status.HTTP_200_OK,
        ),
    ):
...
def test_phone_number_verification_recaptcha(
    ...
    url = reverse("api-internal:user-get-verification-code", kwargs={"pk": user.public_primary_key})
    with patch("apps.api.views.v1.user.check_recaptcha_internal_api", return_value=recaptcha_testing_pass):
