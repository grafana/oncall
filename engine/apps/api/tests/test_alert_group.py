...
def test_get_filter_escalation_chain(
    ...
    response = client.get(url + f"?escalation_chain={escalation_chain.public_primary_key}")
    assert response.status_code == status.HTTP_200_OK
    assert len(response.data["results"]) == 1
    assert response.data["results"][0]["id"] == alert_group.public_primary_key


@pytest.mark.django_db
def test_get_filter_escalation_chain_multiple(
    make_organization_and_user_with_plugin_token,
    make_alert_receive_channel,
    make_channel_filter,
    make_escalation_chain,
    ...
    channel_filter_2 = make_channel_filter(alert_receive_channel, escalation_chain=escalation_chain_2, is_default=True)
