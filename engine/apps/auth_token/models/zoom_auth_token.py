...
class ZoomAuthToken(BaseAuthToken):
    """Auth token for Zoom OAuth flow."""

    user = models.OneToOneField(
        "user_management.User",
        related_name="zoom_auth_token",
        on_delete=models.CASCADE,
    )
    organization = models.ForeignKey(
        "user_management.Organization",
        related_name="zoom_auth_token_set",
        on_delete=models.CASCADE,
    )
    expire_date = models.DateTimeField(default=get_expire_date)

    @classmethod
    def create_auth_token(cls, user: User, organization: Organization) -> Tuple["ZoomAuthToken", str]:
        """Create a new auth token for Zoom OAuth flow."""
