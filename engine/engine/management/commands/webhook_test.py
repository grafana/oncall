...
from apps.webhooks.tasks.send_webhook_event import send_webhook_event


class Command(BaseCommand):
    def handle(self, *args, **options):
        Webhooks = apps.get_model("webhooks", "Webhook")
        webhook = Webhooks.objects.first()
        if not webhook:
            webhook = Webhook()

        webhook.name = "Test webhook"
        webhook.trigger_type = Webhook.TRIGGER_ACKNOWLEDGE
        webhook.http_method = "POST"
        webhook.url = "https://webhook.site/15dc32a7-8d53-4a84-b41b-a290b9fd48e2"
        webhook.save()

        send_webhook_event.apply_async((Webhook.TRIGGER_ACKNOWLEDGE, {"name": "Test 123"}))
