...
import { alertgroupsRetrieve } from 'network/api/autogenerated/onCallApiComponents';
import 'network/api/custom.types';
...
export class AlertGroupStore extends BaseStore {
  ...
  async getAlert(pk: AlertGroup['pk']) {
    ...
    // return data;
    const alertGroup = await alertgroupsRetrieve({ pathParams: { id: pk } });
    runInAction(() => this.alerts.set(pk, alertGroup));
    return alertGroup;
  }
