    ...
    queries: [
      {
        editorMode: 'code',
        exemplar: false,
        expr: `sort_desc(delta(max_over_time(sum by(integration) (avg without(pod, instance)($alert_groups_total{slug=~"${stack}", team=~"$team", integration=~"$integration", service_name=~"$service_name"}))[1h:])[$__range:])>=0)`,
        format: 'table',
        instant: true,
        legendFormat: '__auto',
        range: false,
