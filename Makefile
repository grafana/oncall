...
# compose profiles
MYSQL_PROFILE = mysql
MYSQL_EXTERNAL_PROFILE = mysql_external
...
ifeq ($(findstring $(MYSQL_PROFILE),$(COMPOSE_PROFILES)),$(MYSQL_PROFILE))
	DB=$(MYSQL_PROFILE)
else ifeq ($(findstring $(MYSQL_EXTERNAL_PROFILE),$(COMPOSE_PROFILES)),$(MYSQL_EXTERNAL_PROFILE))
	DB=$(MYSQL_PROFILE)
...
DOCKER_COMPOSE_ENV_VARS := COMPOSE_PROFILES=$(COMPOSE_PROFILES) DB=$(DB) BROKER_TYPE=$(BROKER_TYPE) ENGINE_PORT=$(ENGINE_PORT)
